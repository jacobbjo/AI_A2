...
class Problem:
    ...
    def __init__(self, filename, trajectory_file):
        self.data = self.read_file(filename)
        self.trajectory = self.read_file(trajectory_file)

        self.bounding_polygon = Polygon(self.data["bounding_polygon"])
        self.formation_positions = np.array(self.data["formation_positions"])
        ...
        self.leader_theta = self.trajectory["theta"]
        self.leader_t = self.trajectory["t"]
        self.leader_positions = []

        for i in range(len(self.leader_theta)):
            self.leader_positions.append(np.array([self.trajectory["x"][i], self.trajectory["y"][i]]))
    ...
    def valid_point(self, point):
        """Determines if the point is within the outer polygon"""

        return self.bounding_polygon.contain_point(point)
